...
import { Typography, Toolbar, AppBar, Box, Stack, Container, useMediaQuery } from '@mui/material';
...
export const Header = () => {
  ...
  return (
    <Box height={isMobile ? '56px' : '64px'} mb={isMobile ? '32px' : '48px'} component={'header'}>
      <AppBar position="fixed">
        <Container maxWidth={'xl'}>
          <Toolbar sx={{ justifyContent: 'space-between', color: 'white' }} disableGutters>
            ...
            <Stack direction={'row'} spacing={'16px'} sx={{ color: 'white' }}>
              {links.map(({ title, path, icon }) => {
                ...
                return (
                  <Box key={title} role={'button'} sx={{ cursor: 'pointer' }} onClick={() => logout()}>
                    <NavLinkContent />
                  </Box>
                );
              })}
